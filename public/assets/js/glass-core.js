...
function get_form_query_string(form_id) {
  ...
  $('#' + form_id)
    ...
    .each(function (key) {
      ...
      query_string =
        query_string + '&' + $(this).attr('id') + '=' + encodeURIComponent(val)
    })
  return query_string
}

function save_config() {
  form_data = new FormData(document.getElementById('glass_settings_form'))

  $.ajax({
    type: 'POST',
    url: '/glass_settings_save',
    data: form_data,
    processData: false,
    contentType: false,
  }).then(function (data) {
...
$(document).on('keypress', '#lease_search_criteria', function (e) {
  if (e.which == 13) {
    ...
    $.post(
      '/dhcp_lease_search',
      { search: $('#lease_search_criteria').val() },
      function (result) {
