...
app.use("/dhcp6_leases", require("./routes/dhcp6_leases"));
app.use("/dhcp_lease_search", require("./routes/dhcp_lease_search"));
app.use("/dhcp6_lease_search", require("./routes/dhcp6_lease_search"));
...
app.use("/dhcpv6_pools", require("./routes/dhcpv6_pools"));
app.use("/dhcp_config_snapshots", require("./routes/dhcp_config_snapshots"));
app.use(
  "/dhcp_config_snapshot_view",
  require("./routes/dhcp_config_snapshot_view"),
);
app.use("/dhcp_config_save", require("./routes/dhcp_config_save"));
app.use("/dhcp6_config_save", require("./routes/dhcp6_config_save"));
app.use("/dhcpv4_pools_save", require("./routes/dhcpv4_pools_save"));
app.use("/dhcpv6_pools_save", require("./routes/dhcpv6_pools_save"));
app.use(
  "/dhcp_start_stop_restart",
  require("./routes/dhcp_start_stop_restart"),
);
...
app.use(
  "/glass_alert_settings_save",
  require("./routes/glass_alert_settings_save"),
);
...
try {
  ...
  global.host_name = execSync("/usr/bin/env hostname -s")
    .toString()
    .replace("\n", "");
}
...
wss.on("close", function close() {
  socket_clients--;
  console.log(
    "[WS] CLIENT_DISCONNECT: Socket clients (" + socket_clients + ")",
  );
});
...
setTimeout(function () {
  ...
  let alert_check_timer = setInterval(function () {
    ...
    if (glass_config.leases_per_minute_threshold > 0) {
      ...
      if (
        ...
        slack_message(
          ":warning: CRITICAL: DHCP leases per minute have dropped below threshold " +
            "(" +
            parseInt(glass_config.leases_per_minute_threshold).toLocaleString(
              "en",
            ) +
            ") " +
            "Current (" +
            parseInt(leases_per_minute).toLocaleString("en") +
            ")",
        );

        email_alert(
          "CRITICAL: Leases Per Minute Threshold",
          "DHCP leases per minute dropped below critical threshold <br><br>" +
            "Threshold: (" +
            parseInt(glass_config.leases_per_minute_threshold).toLocaleString(
              "en",
            ) +
        ...
        slack_message(
          ":white_check_mark: CLEAR: DHCP leases per minute have returned to above threshold " +
            "(" +
            parseInt(glass_config.leases_per_minute_threshold).toLocaleString(
              "en",
            ) +
        ...
        email_alert(
          "CLEAR: Leases Per Minute Threshold",
          "DHCP leases per minute have returned to normal <br><br>" +
            "Threshold: (" +
            parseInt(glass_config.leases_per_minute_threshold).toLocaleString(
              "en",
            ) +
  ...
  let alert_subnet_check_timer = setInterval(function () {
    ...
    if (
      glass_config.shared_network_warning_threshold > 0 ||
      ...
      for (var i = 0; i < dhcp_data["shared-networks"].length; i++) {
        utilization = round(
          parseFloat(
            dhcp_data["shared-networks"][i].used /
              dhcp_data["shared-networks"][i].defined,
          ) * 100,
          2,
        );
        ...
        if (
          typeof alert_status_networks_warning[
            dhcp_data["shared-networks"][i].location
          ] === "undefined"
        )
          alert_status_networks_warning[
            dhcp_data["shared-networks"][i].location
        ...
        if (
          typeof alert_status_networks_critical[
            dhcp_data["shared-networks"][i].location
          ] === "undefined"
        )
          alert_status_networks_critical[
        ...
        if (glass_config.shared_network_warning_threshold > 0) {
          if (
            ...
            slack_message(
              ":warning: WARNING: DHCP shared network utilization (" +
                dhcp_data["shared-networks"][i].location +
                ") " +
                "Current: (" +
                utilization +
                "%) " +
                "Threshold: (" +
                glass_config.shared_network_warning_threshold +
                "%)",
            );

            email_alert(
              "WARNING: DHCP shared network utilization",
              "WARNING: DHCP shared network utilization (" +
                dhcp_data["shared-networks"][i].location +
                ") <br><br>" +
                "Threshold: (" +
            ...
            slack_message(
              ":white_check_mark: CLEAR: Warning DHCP shared network utilization (" +
                dhcp_data["shared-networks"][i].location +
                ") " +
                "Current: (" +
                utilization +
            ...
            email_alert(
              "CLEAR: DHCP shared network utilization warning",
              "CLEAR: DHCP shared network utilization (" +
                dhcp_data["shared-networks"][i].location +
                ") <br><br>" +
                "Threshold: (" +
                glass_config.shared_network_warning_threshold +
        ...
        if (glass_config.shared_network_critical_threshold > 0) {
          if (
            ...
            slack_message(
              ":fire: CRITICAL: DHCP shared network utilization (" +
                dhcp_data["shared-networks"][i].location +
                ") " +
                "Current: (" +
                utilization +
                "%) " +
                "Threshold: (" +
                glass_config.shared_network_critical_threshold +
                "%)",
            );

            email_alert(
              "CRITICAL: DHCP shared network utilization",
              "CRITICAL: DHCP shared network utilization (" +
                dhcp_data["shared-networks"][i].location +
                ") <br><br>" +
                "Threshold: (" +
            ...
            slack_message(
              ":white_check_mark: CLEAR: Critical DHCP shared network utilization (" +
                dhcp_data["shared-networks"][i].location +
                ") " +
                "Current: (" +
                utilization +
...
function email_alert(alert_title, alert_message) {
  if (
    typeof glass_config.email_alert_to === "undefined" &&
    typeof glass_config.sms_alert_to === "undefined"
  ) {
    console.log(
      "[Glass Server] E-Mail alert triggered, but no addresses configured...",
    );
    return false;
  }
  ...
  glass_config.email_alert_to = glass_config.email_alert_to
    .replace(/^[,\s]+|[,\s]+$/g, "")
    .replace(/,[,\s]*,/g, ",");
