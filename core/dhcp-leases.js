...
module.exports = {
  ...
  parseV6LeasesFileOnce: function (glass_config) {
    fs.readFile(glass_config.v6_leases_file, 'utf8', function (err, data) {
      if (err) {
        return console.log(err)
      } else {
        lease_parser.parseV6(data)
        console.log('[Glass Server] v6 Leases file loaded')
      }
    })
  },
  startLeaseListener: function (glass_config) {
    ...
    let file_tail = new tail(glass_config.leases_file, '\n', options)

    console.log(
      "[Glass Server] Watching leases file '" + glass_config.leases_file + "'"
    )
  ...
  startV6LeaseListener: function (glass_config) {
    var v6_lease_read_buffer = ''
    var options = {}
    options.interval = 1000

    let file_tail = new tail(glass_config.v6_leases_file, '\n', options)

    console.log(
      "[Glass Server] Watching v6 leases file '" +
    ...
    file_tail.on('line', function (data) {
       ...
       * Buffering lines until we get full lease data
       */
      v6_lease_read_buffer = v6_lease_read_buffer + data + '\n'

      /**
       * End of lease - cut off and parse the buffer
       */
      if (/}/i.test(data)) {
        lease_parser.parseV6(v6_lease_read_buffer)
        v6_lease_read_buffer = ''
      }
