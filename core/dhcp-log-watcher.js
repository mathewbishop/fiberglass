...
module.exports = {
  init: function (glass_config) {
    ...
    var tail_dhcp_log = new tail(glass_config.log_file, '\n', options)
      ...
      if (/DHCPREQUEST/i.test(data)) {
        ...
        if (typeof request_from !== 'undefined') {
          if (request_from.length === 17 && /:/.test(request_from)) {
            ...
            if (
              typeof dhcp_requests[request_from].request_vendor === 'undefined'
            )
  ...
  init6: function (glass_config) {
    console.log('[Glass Server] DHCP6 log watcher initialized')

    var options = {}
    options.interval = 300
    var tail_dhcp_log = new tail(glass_config.v6_log_file, '\n', options)
