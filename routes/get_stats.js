...
router.get('/', authCheck, function (req, res, next) {
  ...
  for (var i = 0; i < dhcp_data['shared-networks'].length; i++) {
    ...
    table_row =
      table_row +
      '<td>' +
      dhcp_data['shared-networks'][i].used.toLocaleString('en') +
      ' (' +
      utilization +
      '%)</td>'
    table_row =
      table_row +
      '<td class="hide_col">' +
      dhcp_data['shared-networks'][i].defined.toLocaleString('en') +
      '</td>'
    table_row =
      table_row +
      '<td class="hide_col">' +
      dhcp_data['shared-networks'][i].free.toLocaleString('en') +
      '</td>'

    utilization_color = 'green'

    if (utilization >= 80) utilization_color = 'orange'
    if (utilization >= 90) utilization_color = 'red'
    ...
    table_row =
      ...
      utilization_color +
      '" role="progressbar" aria-valuenow="62" aria-valuemin="0" aria-valuemax="100" style="width: ' +
      utilization +
      '%"></div>' +
      '</div></td>'
  ...
  for (var i = 0; i < dhcp_data.subnets.length; i++) {
    utilization = round(
      parseFloat(dhcp_data.subnets[i].used / dhcp_data.subnets[i].defined) *
        100,
      2
    )
    if (isNaN(utilization)) utilization = 0
  ...
  for (var i = 0; i < dhcp_data.subnets.length; i++) {
    ...
    table_row = ''
    table_row =
      table_row + '<td><b>' + dhcp_data.subnets[i].location + '</b></td>'
    table_row = table_row + '<td>' + dhcp_data.subnets[i].range + '</td>'
    table_row =
      table_row +
      '<td>' +
      dhcp_data.subnets[i].used.toLocaleString('en') +
      ' (' +
      utilization +
      '%)</td>'
    table_row =
      table_row +
      '<td class="hide_col">' +
      dhcp_data.subnets[i].defined.toLocaleString('en') +
      '</td>'
    table_row =
      table_row +
      '<td class="hide_col">' +
      dhcp_data.subnets[i].free.toLocaleString('en') +
      '</td>'
