...
router.post('/', checkUser, function (req, res, next) {
  ...
  for (var key in dhcp_lease_data) {
    ...
    table_row =
      table_row + '<td>' + dhcp_lease_data[key].mac_oui_vendor + '</td>'
    table_row =
      table_row +
      '<td>' +
      (dhcp_lease_data[key].host ? dhcp_lease_data[key].host : '') +
      '</td>'
    table_row =
      table_row +
      '<td>' +
      human_time(dhcp_lease_data[key].start * 1000) +
      '</td>'
    table_row =
      table_row + '<td>' + human_time(dhcp_lease_data[key].end * 1000) + '</td>'
    if (typeof dhcp_lease_data[key].mac !== 'undefined') {
      table_row =
        table_row +
        '<td>' +
        '<button class="btn btn-default waves-effect option_data" lease="' +
        dhcp_lease_data[key].mac.split(':').join('') +
        '">Show</button>' +
