    ...
    function (err, stdout, stderr) {
      ...
      if (err) {
        ...
        output = output.replace(
          ...
          '<br><br>Configuration file errors encountered'
        )

        res.send(
          '<script type="text/javascript">modal (\'DHCP Config Syntax Validation\', ' +
            JSON.stringify('<span style="color:red">' + output + '</span>') +
            ', "");'
        )
        ...
        output = output.replace('\n', '<br>')
        res.send(
          '<script type="text/javascript">modal (\'DHCPv6 Pools Save\', ' +
